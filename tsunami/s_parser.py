...
def parse_args():
    ...
    # Additions
    parser.add_argument("--local_flag",default=False,type=str2bool) #indicates if on local or gpu
    parser.add_argument("--num_dims", default=None, type=int) #for unstructured data/point clouds- used in positional encoder and dataloader
    parser.add_argument("--data_key",default=None, type=str) #to pass a specific .mat file in tsunami directory
    parser.add_argument("--lam", default=0.00, type=float)  #soft constraint scale
    parser.add_argument("--unseen_flag", default=None, type=str2bool) #indicates if data is seen or unseen
    parser.add_argument("--bath_sampling",default=10, type=int) #positional encoding for bathymetry
    parser.add_argument("--path_pref",default=None,type=str) #add this to send figs to a user specified directory
    ...
    #[data_config['gpu_device']] if 
    #     isinstance(data_config['gpu_device'],int) else None
    ...
    data_config = dict(data_name = args.data_name,
                       ...
                       local_flag=args.local_flag,
                       num_dims=args.num_dims,
                       data_key=args.data_key,
                       lam=args.lam,
                       unseen_flag=args.unseen_flag,
                       bath_sampling=args.bath_sampling,
                       path_pref=args.path_pref
                       )
